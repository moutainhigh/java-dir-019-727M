package com.yaolong.rbac.commons.exceptions;
...
public class BusinessExceptionHandler {
	...
	@ExceptionHandler({ BusinessException.class, Exception.class, ConstraintViolationException.class })
	public ResponseEntity<?> handlerException(HttpServletRequest request, Exception ex) {
		Map<String, Object> error = new HashMap<>(2);
		System.out.println("当前的异常:"+ex);
		// 业务异常
		...
		// 统一处理请求参数格式错误 @RequestParam上validate
		else if (ex instanceof ConstraintViolationException) {
			ConstraintViolationException constraintViolationException = (ConstraintViolationException) ex;
			String msg = constraintViolationException.getConstraintViolations().stream().map(ConstraintViolation::getMessage).collect(Collectors.joining());
			error.put("code", HttpStatus.BAD_REQUEST.value());
			error.put("message", msg);
		}
		// 未知错误
