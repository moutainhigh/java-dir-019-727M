...
import com.yaolong.rbac.jwt.mapper.SysMenuResourceVbenMapper;
...
@Service
@Slf4j
public class SysMenuResourceVbenServiceImpl extends BaseServiceImpl<SysMenuResourceVbenMapper, SysMenuResourceVben> implements ISysMenuResourceVbenService {



    @Resource
    public ISysRoleMenuResourceVbenService studyRoleMenuResourceVbenService;

    @Resource
    public ISysMenuMetaService studyMenuMetaService;

    @Override
    public IPage<?> page(int current, int size, SysMenuResourceVben domain) {
    ...
    public boolean update(SysMenuResourceVben domain) {
    ...
    @Override
    @Transactional(rollbackFor = BusinessException.class)
    public boolean create(SysMenuResourceVben domain) {
    ...
    public boolean remove(Long id) {
        if (checkId(id)){
            SysMenuResourceVben studyMenuResource = super.get(id);

            //删除角色菜单关系
            QueryWrapper<SysRoleMenuResourceVben> queryWrapper = new QueryWrapper<>();
            ...
            // 判断是否存在子菜单
            List<SysMenuResourceVben> list = super.list();
            list.forEach(item ->{
     ...
     * @param menuItems
     * @return java.util.List<com.yaolong.rbac.jwt.po.vbenAdmin.MenuItem>
     * @author yaolong
     * @date 2021/8/6 10:41
     */
