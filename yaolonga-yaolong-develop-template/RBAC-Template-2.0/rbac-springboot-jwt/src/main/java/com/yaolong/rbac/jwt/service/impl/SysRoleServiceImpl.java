...
public class SysRoleServiceImpl extends BaseServiceImpl<SysRoleMapper, SysRole> implements ISysRoleService {
    ...
    @Resource
    ISysRoleMenuResourceService sysRoleMenuResourceVbenService;

    @Resource
    ISysMenuResourceService sysMenuResourceVbenService;
    ...
    private RoleParam allotIdsHandler(RoleParam param) {
        //================菜单id处理 （获取父id）===============
        List<SysMenuResource> menuResources = sysMenuResourceVbenService.list();
     ...
     */
    private List<Long> getMenuPIds(List<SysMenuResource> menuResources, List<Long> menuIds) {
        Stream<SysMenuResource> ids = menuResources.stream().filter(m -> menuIds.contains(m.getId()));
        List<Long> menuIds2 = ids.map(SysMenuResource::getParentId).filter(r -> r != 0)
    ...
    public boolean remove(Long id) {
        if (checkId(id)) {
            ...
            deptQueryWrapper.eq("role_id", id);
            ...
            sysRoleMenuResourceVbenService.remove(roleMenuQueryWrapper);
