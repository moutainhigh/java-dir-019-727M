package com.ubiquitous.market.admin.api.api.order;
...
import com.ubiquitous.market.core.ratelimit.annotation.RateLimiter;
...
import com.ubiquitous.market.data.enums.UserLoginType;
...
//    IOrderMessageService iOrderMessageService;
    ...
    @Value("${com.ubiquitous.market.wx.mini.app-id}")
    private String wxMiNiAppid;

    @Value("${com.ubiquitous.market.wx.app.app-id}")
    ...
    public Boolean distributeOrder(OrderDTO orderDTO, Long riderId) throws AppServiceException {
        ...
        try {
            ...
            orderMessageBO.setStoreId(orderDOStoreId.intValue());
            // 优惠价 = 实际价格 - 原价（不是以前的原价）
            Integer prefer = orderDO.getActualPrice() - orderDO.getSkuTotalPrice();
            orderMessageBO.setTotalPreferentialPrice(prefer.doubleValue() / 100);
            orderMessageBO.setTotalPrice(orderDO.getSkuTotalPrice().doubleValue() / 100);
            orderMessageBO.setGmtOrderCreate(orderDO.getGmtCreate());
            RiderSpuBO riderSpuBO;
            List<RiderSpuBO> riderSpuBOList = new ArrayList<>();
            for (OrderSkuDO orderSkuDO : orderSkuDoList) {
                riderSpuBO = new RiderSpuBO();
                riderSpuBO.setSpuName(orderSkuDO.getTitle());
                riderSpuBO.setAmount(String.valueOf(orderSkuDO.getNum().doubleValue()));
                riderSpuBO.setPreferentialPrice((orderSkuDO.getPrice().doubleValue() - orderSkuDO.getOriginalPrice().doubleValue()) / 100);
                riderSpuBO.setUrl(orderSkuDO.getImg());
                riderSpuBO.setUnitPrice(orderSkuDO.getPrice().doubleValue() / 100);
                riderSpuBOList.add(riderSpuBO);
            }
            orderMessageBO.setRiderSpuBOList(riderSpuBOList);
            orderMessageBO.setPredictTime(orderDO.getGmtPredict());
            BeanUtils.copyProperties(orderDO, orderMessageBO);
            orderMessageBO.setRiderId(riderId);
            // 更新订单配送员信息
            OrderDO updateOrderDO = new OrderDO();
