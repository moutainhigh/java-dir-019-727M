package com.ubiquitous.market.rider.controller;
...
import com.ubiquitous.market.rider.manager.ApiManager;
import com.ubiquitous.market.rider.model.GatewayResponse;
    ...
    private Object process(HttpServletRequest request, HttpServletResponse response, long invokeTime) throws ServiceException {
        try {
            ...
            for (int i = 0; i < methodParameters.length; i++) {
                ...
                if (httpParam.type() == HttpParamType.COMMON) {
                    ...
                    if (!StringUtils.isEmpty(accessToken)) {
                        ...
                        if (!StringUtils.isEmpty(userJson)) {
                            ...
                            userRedisTemplate.expire(Const.USER_REDIS_PREFIX + accessToken, 60, TimeUnit.MINUTES);
                            continue;
                        }
                    }
                ...
                } else if (httpParam.type() == HttpParamType.RIDER_ID) {
                    String accessToken = request.getHeader(Const.RIDER_ACCESS_TOKEN);
                    if (!StringUtils.isEmpty(accessToken)) {
                        String userJson = userRedisTemplate.opsForValue().get(Const.RIDER_REDIS_PREFIX + accessToken);
                        if (!StringUtils.isEmpty(userJson)) {
                            UserDTO userDTO = JSONObject.parseObject(userJson, UserDTO.class);
                            SessionUtil.setUser(userDTO);
                            args[i] = userDTO.getId();
                            userRedisTemplate.expire(Const.RIDER_REDIS_PREFIX + accessToken, 120, TimeUnit.MINUTES);
                            continue;
                        }
                    }
                    if (args[i] == null && methodParam.getAnnotation(NotNull.class) != null) {
                    ...
                    if (!StringUtils.isEmpty(accessToken)) {
                        ...
                        if (!StringUtils.isEmpty(userJson)) {
                            ...
                            userRedisTemplate.expire(Const.ADMIN_REDIS_PREFIX + accessToken, 60, TimeUnit.MINUTES);
                            continue;
                        }
                    }
                    if (args[i] == null && methodParam.getAnnotation(NotNull.class) != null) {
