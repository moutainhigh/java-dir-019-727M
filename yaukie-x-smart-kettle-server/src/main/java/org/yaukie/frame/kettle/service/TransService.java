...
public class TransService  {
     ...
     * @Destrib:  启动一个转换
     **/
    public void startTrans(XTrans xTrans ,Map params) throws XtlExceptions, KettleException {
     ...
     * @Destrib:  运行一个资源库转换
    **/
        private void doRunRepoTrans(XTrans xTrans,Map params )  {
        ...
        if (xRepository == null) {
            ...
            TransMeta transMeta = KettleUtil.loadTrans(xTrans.getTransName(),xTrans.getTransPath(),repository) ;
                ...
                if(!CollectionUtils.isEmpty(params)){
                    log.info("运行转换["+transMeta.getName()+"]携带参数={}", JSON.toJSON(params));
                    params.forEach((k,v) -> {
                        try {
                            transMeta.setParameterValue(k.toString(),v.toString());
     ...
     * @Destrib:  运行一个文件库转换
    **/
    private void doRunFileTrans(XTrans xTrans,Map params) throws KettleException {
        String repositoryId = xTrans.getTransRepositoryId();
        XRepositoryExample xRepositoryExample = new XRepositoryExample();
        xRepositoryExample.createCriteria().andRepoIdEqualTo(repositoryId + "");
        XRepository xRepository = xRepositoryService.selectFirstExample(xRepositoryExample);
