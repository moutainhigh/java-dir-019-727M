...
public class JobService  {
     ...
     * @Destrib: 启动一个作业
     **/
    public Job startJob(XJob xJob,Map params) throws XtlExceptions, KettleException {
        ...
        if (jobType.equalsIgnoreCase(Constant.JOB_FILE_TYPE)) {
            return doRunFileJob(xJob,params);
            /**如果是资源库中的作业*/
        } else if (jobType.equalsIgnoreCase(Constant.JOB_REPO_TYPE)) {
           return doRunRepoJob(xJob,params);
        }else if(jobType.equalsIgnoreCase("sftp")){
            return doRunFileJobFromFtp(xJob);
        }
     ...
     **/
    private Job doRunRepoJob(XJob xJob,Map params) throws XtlExceptions, KettleException {
        ...
        if(!CollectionUtils.isEmpty(params)){
            log.info("运行作业["+jobMeta.getName()+"]携带参数={}", JSON.toJSON(params));
                  params.forEach((k,v) -> {
                    try {
                        jobMeta.setParameterValue(k.toString(),v.toString());
     ...
     **/
    private Job doRunFileJob(XJob xJob,Map params) throws XtlExceptions, KettleException {
        String repositoryId = xJob.getJobRepositoryId();
        XRepositoryExample xRepositoryExample = new XRepositoryExample();
        xRepositoryExample.createCriteria().andRepoIdEqualTo(repositoryId + "");
        XRepository xRepository = xRepositoryService.selectFirstExample(xRepositoryExample);
     ...
     */
    private Job doRunFileJobFromFtp(XJob xJob) throws XtlExceptions, KettleException {
        String repositoryId = xJob.getJobRepositoryId();
        XRepositoryExample xRepositoryExample = new XRepositoryExample();
        xRepositoryExample.createCriteria().andRepoIdEqualTo(repositoryId + "");
        XRepository xRepository = xRepositoryService.selectFirstExample(xRepositoryExample);
    ...
    public XJobStatus doStopJob(XJob xJob) {
        ...
        try {
            ...
            log.info("作业是否被中断,{}",job.isInterrupted());
            KettleUtil.jobStopAll(job);
            log.info("作业是否被中断,{}",job.isInterrupted());
           log.info("作业 {} 成功停止,作业状态{}!", xJob.getJobName(), job.getStatus());
    ...
    public XJobStatus doKillJob(XJob xJob) throws XtlExceptions {
        ...
        try {
           ...
           log.info("作业 {} 被强行终止,作业状态{}!", xJob.getJobName(), job.getStatus());
            return KettleUtil.getJobStatus(job);
        } catch (XtlExceptions e) {
